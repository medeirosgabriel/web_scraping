...
def get_nome(response):
  dep_nome = response.css('div.l-identificacao-info>ul.informacoes-deputado li::text').getall()[0].replace("\n", "").strip()
  return dep_nome

def get_nascimento(response):
  try:
    nascimento = response.css('div.l-identificacao-info>ul.informacoes-deputado li::text').getall()[4].replace("\n", "").strip()
  except:
    nascimento = response.css('div.l-identificacao-info>ul.informacoes-deputado li::text').getall()[1].replace("\n", "").strip()
  return nascimento
...
def get_presenca(response):
  ...
  if (presencas != []):
    ...
    plenario = list(map(lambda x: x.replace("dias", "").replace("\n", "").replace(" ", ""), plenario))
    ...
    comissao = list(map(lambda x: x.replace("reuniões", "").replace("\n", "").replace(" ", ""), comissao))
...
def get_gasto_total_parlamento(response):
  gasto = response.css("table[id=percentualgastocotaparlamentar]>tbody>tr>td::text").getall()
  try:
...
def get_gasto_total_gabinete(response):
  gasto = response.css("table[id=percentualgastoverbagabinete]>tbody>tr>td::text").getall()
